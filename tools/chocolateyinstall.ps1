...
Get-ChocolateyUnzip -File "$toolsDir\en_5.5.708.zip" -Destination "$toolsDir";

#$setupLocation = Join-Path $toolsDir 'en_5.5.708'
$fileLocation = Join-Path $toolsDir 'en_5.5.708\setup.exe'
$issFileLocation = Join-Path $toolsDir 'en_5.5.708\en_5.5.708_install.iss'
...
#  url64bit      = $url64
  ...
  softwareName  = 'DesignPro*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
  ...
  # MSI
  silentArgs    = '/s /f1"$issFileLocation"' # ALLUSERS=1 DISABLEDESKTOPSHORTCUT=1 ADDDESKTOPICON=0 ADDSTARTMENU=0
